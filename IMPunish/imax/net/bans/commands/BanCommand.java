...
public class BanCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender s, Command cmd, String label, String[] args) {
        ...
        if (!s.hasPermission("street.bans") && !s.hasPermission("street.bans.ajudantes")){
            s.sendMessage("§cSem permissão.");
            return false;
        }
        OfflinePlayer p = Bukkit.getOfflinePlayer(args[0]);

        if (DBManagement.getAPI().isPunished(p.getUniqueId(), PunishType.BAN) || DBManagement.getAPI().isPunished(p.getUniqueId(), PunishType.BAN_PERMA)){
            s.sendMessage("§cEsse jogador já está punido.");
            return false;
        }

        BandMType type = new BandMType(p.getUniqueId(), p.getName(), PunishType.BAN);
